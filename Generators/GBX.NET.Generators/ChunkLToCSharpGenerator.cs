...
public class ChunkLToCSharpGenerator : SourceGenerator
{
    ...
    public override void Execute(GeneratorExecutionContext context)
    {
        ...
        foreach (var chunkLFile in Directory.GetFiles(enginesDir, "*.chunkl", SearchOption.AllDirectories))
        {
            ...
            _ = engineTypes.TryGetValue($"{ns}::{chunkL.ClassName}", out var existingTypeSymbol);

            var classBuilder = new ChunkLClassBuilder(ns, chunkL, existingTypeSymbol);
            var code = classBuilder.Build();

            context.AddSource(chunkL.ClassName, code);
        }
    }
}
